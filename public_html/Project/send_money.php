...
<?php
$display = $pdo->prepare("SELECT * FROM Accounts WHERE user_id <> '3' AND user_id <> :user AND account_type != 'loan' ");
$display->bindValue(':user', $user_id);
$display->execute();
$display->setFetchMode(PDO::FETCH_ASSOC);
echo '
...
if(isset($_POST['submit'])){
    ...
    if($isFrozen == 1){
        $hasError = true;
        flash('Cannot use transactions when either accounts are frozen');
    }
